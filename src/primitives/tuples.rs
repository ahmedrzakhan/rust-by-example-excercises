...
// matrix as an argument, and returns a matrix in which two elements have been swapped. For example:
// println!("Matrix:\n{}", matrix);
// println!("Transpose:\n{}", transpose(matrix));
...
fn reverse_numbers(pair: (f32, f32)) -> (f32, f32) {
    let (first, second) = pair;
    (second, first)
}

fn transpose(mat: Matrix) -> Matrix {
    let (a, b) = reverse_numbers((mat.1, mat.2));
